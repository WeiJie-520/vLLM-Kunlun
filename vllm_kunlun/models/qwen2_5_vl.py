    ...
    def __init__(self, dim: int, theta: float = 10000.0) -> None:
        ...
        初始化函数，用于创建一个实例。
            ...
            Args:
                ...
                theta (float, optional, default=10000.0): 参数，控制频谱分布的平滑程度，默认为10000.0。
...
class Qwen2_5_VLForConditionalGeneration(
    nn.Module, SupportsMultiModal, SupportsLoRA, SupportsPP, SupportsQuant
):
    ...
    def _process_image_input(
        ...
        Args:
            image_input (Qwen2_5_VLImageInputs): 包含图像信息的字典，其中包括以下键值对：
                - type (str, optional): 图像类型，可选值为"image_embeds"或者None（默认），表示使用图像嵌入；
                   None时，需要提供"pixel_values"和"image_grid_thw"键值对。
                - pixel_values (torch.Tensor, optional): 图像像素值，shape为(batch_size, num_channels, height, width)，
                   dtype为float32，optional，只有当type为None时才需要提供。
                - image_grid_thw (torch.Tensor, optional): 图像网格大小，shape为(batch_size, 2)，dtype为int64，
                   optional，只有当type为None时才需要提供。
        ...
        sizes = (grid_thw[:, 0] * grid_thw[:, 1] * grid_thw[:, 2]) // (
    ...
    def _process_video_input(
        ...
        Args:
            video_input (Qwen2_5_VLVideoInputs): 包含视频数据的字典，包括：
                - type (str): 类型，可选值为"video_embeds"或"pixel_values_videos"。
                - video_grid_thw (torch.Tensor): 视频网格大小，形状为(N, 2)，其中N是视频项数，第一维度表示视频项索引，最后两个维度分别代表高和宽。
                - video_embeds (Optional[torch.Tensor]): 视频嵌入，仅当type为"video_embeds"时有效。
                - pixel_values_videos (Optional[torch.Tensor]): 像素值视频，仅当type为"pixel_values_videos"时有效。

        Returns:
            tuple[torch.Tensor, ...]: 元组，包含每个视频项的张量，长度为N。
        """
