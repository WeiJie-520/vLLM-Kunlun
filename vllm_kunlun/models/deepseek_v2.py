...
def sparse_attn_indexer_vllm_kunlun(
    ...
    if has_prefill:
        ...
        for chunk in prefill_metadata.chunks:
            ...
            del k_fp8, k_scale
            topk_indices = torch.argsort(logits, dim=-1, descending=True)[
    ...
    if has_decode:
        ...
        del positions, mask
        topk_indices = torch.ops._C.fast_topkv2(
            logits, decode_metadata.seq_lens, topk_tokens
        )  # [B * N, K]
        need_mask = decode_metadata.seq_lens_cpu.min() < topk_tokens
        if need_mask:
            positions_topk = (
                torch.arange(topk_tokens, device=current_device)
                .unsqueeze(0)
                .expand(batch_size * next_n, -1)
            )
