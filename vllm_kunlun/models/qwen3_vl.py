...
class Qwen3_VisionTransformer(nn.Module):
    ...
    def forward(
        ...
        )  # shape: [seq_len, 1, head_dim]
        rotary_pos_emb_sin = repeat(
            rotary_pos_emb_sin,
            "... d -> ... 1 (2 d)" if not interleaved else "... d -> ... 1 (d 2)",
        )  # shape: [seq_len, 1, head_dim]
        rotary_pos_emb_cos_sin_cache = torch.cat(
            [rotary_pos_emb_cos, rotary_pos_emb_sin], dim=1
        )  # shape: [seq_len, 2, head_dim]
        ...
        for layer_num, blk in enumerate(self.blocks):
            hidden_states = blk(
                hidden_states,
                cu_seqlens=cu_seqlens,
                rotary_pos_emb=rotary_pos_emb_cos_sin_cache,
                max_seqlen=max_seqlen,
                seqlens=seqlens,
            )
        ...
        if self.use_deepstack:
            ...
            if image_input is not None or video_input is not None:
                deepstack_input_embeds = (
                    torch.zeros_like(inputs_embeds)
                    .unsqueeze(1)
                    .repeat(1, self.deepstack_num_level, 1)
                    .flatten(1)
                )
        ...
        if image_input is not None:
            ...
            if self.use_deepstack:
                ...
                image_embeds, image_embeds_multiscale = image_embeds.split(
                    [visual_dim, visual_dim * self.deepstack_num_level], dim=-1
                )
        ...
        if self.use_deepstack and deepstack_input_embeds is not None:
            deepstack_input_embeds = (
                deepstack_input_embeds.view(
                    inputs_embeds.shape[0], self.deepstack_num_level, visual_dim
                )
                .permute(1, 0, 2)
                .contiguous()
            )
