        ...
        if is_rocm_aiter_fp8bmm_enabled():
            ...
            if vllm_kunlun_envs.VLLM_KUNLUN_ENABLE_INT8_BMM:
                ...
                sorted_tokens_num_lod = (
                    torch.arange(self.num_heads + 1, dtype=torch.int, device="cuda")
                    * q_len
                )
...
class MLACommonImpl(MLACommonBaseImpl[M], Generic[M]):
    ...
    def _flash_attn_varlen_diff_headdims(
        ...
        **kwargs,
        ...
        if isinstance(attn_out, tuple):
            attn_out, _lse = attn_out[0], attn_out[1]
    ...
    def process_weights_after_loading(self, act_dtype: torch.dtype):
        ...
        if vllm_kunlun_envs.VLLM_KUNLUN_ENABLE_INT8_BMM:
            ...
            assert (
                kv_b_proj_weight.dtype == torch.int8
            ...
            w_uk_dq = W_UK.contiguous().cpu().to(torch.bfloat16).to(
            ...
            self.W_UK_SCALE = torch.empty(
                [W_UK.shape[0] * W_UK.shape[2], 1],
                dtype=torch.float,
